...
<body class="antialiased bg-[#0f172a] min-h-screen flex items-center justify-center p-6">
    <div class="absolute inset-0 z-0 overflow-hidden">
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-blue-600/10 blur-[120px] rounded-full"></div>
    </div>
    ...
    <div class="w-full max-w-md relative z-10">
        ...
        @if(session('success'))
            <div class="mb-6 p-4 rounded-2xl bg-green-500/10 border border-green-500/20 text-green-400 text-sm text-center">
                {{ session('success') }}
            </div>
        @endif
        ...
        <div class="glass p-8 rounded-3xl">
            <div class="space-y-6">
                <div class="text-center">
                    <p class="text-xs uppercase tracking-widest text-slate-500 font-bold mb-8">Masuk dengan Akun Sosial</p>
                </div>
                ...
                <div class="space-y-4">
                    <div class="flex items-start gap-3 p-4 bg-white/5 border border-white/10 rounded-2xl">
                        ...
                        <label for="agree_policy" class="text-xs text-slate-400 cursor-pointer">
                            Saya telah membaca dan menyetujui <a href="{{ route('policy') }}#terms" target="_blank" class="text-blue-400 hover:underline">Ketentuan Layanan</a> serta <a href="{{ route('policy') }}#privacy" target="_blank" class="text-blue-400 hover:underline">Kebijakan Privasi</a> D'JOKI.
                        </label>
                    </div>

                    <div id="login_buttons_container" class="grid grid-cols-1 gap-4 opacity-40 pointer-events-none transition-opacity duration-300">
                        <a href="javascript:void(0)" onclick="loginWithGoogle()" class="flex items-center justify-center px-6 py-4 bg-white/5 border border-white/10 rounded-2xl hover:bg-white/10 transition group hover:border-blue-500/50">
                            <svg class="h-6 w-6 mr-3" viewBox="0 0 24 24" fill="currentColor">
                                <path d="M12.48 10.92v3.28h7.84c-.24 1.84-.908 3.152-1.896 4.14-1.236 1.236-3.156 2.532-6.424 2.532-5.172 0-9.204-4.14-9.204-9.204s4.032-9.204 9.204-9.204c2.82 0 4.932 1.104 6.432 2.52l2.316-2.316C18.42 1.056 15.756 0 12.48 0 6.648 0 1.92 4.728 1.92 10.56S6.648 21.12 12.48 21.12c3.156 0 5.544-1.044 7.416-2.976 1.92-1.92 2.532-4.596 2.532-6.768 0-.648-.048-1.26-.144-1.848h-9.804z"/>
                            </svg>
                            <span class="text-sm font-bold text-white">Lanjutkan dengan Google</span>
                        </a>
                        
                        <a href="{{ route('social.redirect', 'github') }}" class="flex items-center justify-center px-6 py-4 bg-white/5 border border-white/10 rounded-2xl hover:bg-white/10 transition group hover:border-slate-400/50">
                            <svg class="h-6 w-6 mr-3" viewBox="0 0 24 24" fill="currentColor">
        ...
        window.loginWithGoogle = async () => {
            try {
                ...
                if (!response.ok) {
                    ...
                    throw new Error("Gagal verifikasi ke server. Pastikan file kredensial Firebase sudah terpasang di storage/app/firebase-auth.json");
                }
