...
@section('header', 'Edit Metode')
@section('subheader', 'Perbarui detail akun pembayaran.')
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="glass rounded-3xl p-8">
        <form action="{{ route('payment-methods.update', $paymentMethod) }}" method="POST" class="space-y-6">
            ...
            @method('PUT')
            <div>
                <label for="name" class="block text-xs font-bold text-slate-500 uppercase mb-2">Nama Metode</label>
            ...
            <div class="flex items-center space-x-3 p-4 bg-white/5 rounded-2xl">
                ...
                <input type="checkbox" id="is_active" name="is_active" value="1" {{ $paymentMethod->is_active ? 'checked' : '' }}
                       class="h-5 w-5 rounded border-white/10 bg-[#0f172a] text-blue-600 focus:ring-offset-[#0f172a]">
                <label for="is_active" class="text-sm text-slate-300">Aktifkan metode ini.</label>
            </div>
