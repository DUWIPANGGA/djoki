...
@section('subheader', 'Perbarui detail karya terbaik Anda.')
    ...
    <div class="glass rounded-3xl p-8">
        <form action="{{ route('portfolios.update', $portfolio) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid md:grid-cols-2 gap-8">
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" id="title" name="title" required 
                               ...
                               placeholder="Contoh: E-Commerce dengan Laravel" value="{{ old('title', $portfolio->title) }}">
                    ...
                    <div>
                        ...
                        <input type="url" id="external_link" name="external_link" 
                               ...
                               placeholder="https://github.com/username/project" value="{{ old('external_link', $portfolio->external_link) }}">
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="6"
                                  ...
                                  placeholder="Jelaskan fitur, teknologi yang digunakan, dan hasil dari project ini...">{{ old('description', $portfolio->description) }}</textarea>
                ...
                <div class="space-y-6">
                    <div>
                        <label class="block text-sm font-medium text-slate-300 mb-2">Media Saat Ini</label>
                        <div class="grid grid-cols-2 gap-4">
                            @forelse($portfolio->media_files ?? [] as $media)
                                <div class="relative group aspect-video rounded-xl overflow-hidden bg-white/5">
                                    <img src="{{ Storage::url($media) }}" class="w-full h-full object-cover">
                                    <div class="absolute inset-0 bg-red-600/50 opacity-0 group-hover:opacity-100 transition flex items-center justify-center">
                                        <p class="text-[10px] font-bold text-white uppercase">Hapus</p>
                                    </div>
                                </div>
                            @empty
                                <div class="col-span-2 py-4 text-center text-xs text-slate-500 italic">Tidak ada media.</div>
                            @endforelse
                        </div>
                        
                        <label class="block text-sm font-medium text-slate-300 mt-6 mb-2">Tambah Media Baru</label>
                    ...
                    <div class="p-6 glass rounded-2xl">
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="is_public" value="1" class="sr-only peer" {{ $portfolio->is_public ? 'checked' : '' }}>
